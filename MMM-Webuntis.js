  ...
  getDom: function () {
    ...
    for (let studentTitle in this.lessonsByStudent) {
      ...
      for (let i = 0; i < lessons.length; i++) {
        ...
        // skip past lessons
        if (time < new Date() && lesson.code != "error") {
          continue;
        }
        ...
        dateTimeCell.innerHTML =
          time.toLocaleDateString("de-DE", { weekday: "short" }).toUpperCase() +
          "&nbsp;";
        if (this.config.showStartTime || lesson.lessonNumber === undefined) {
          dateTimeCell.innerHTML += time.toLocaleTimeString("de-DE", {
            hour: "2-digit",
            minute: "2-digit"
          });
        } else {
