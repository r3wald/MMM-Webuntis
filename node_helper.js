...
module.exports = NodeHelper.create({
    ...
    if (notification === "FETCH_DATA") {
      ...
      for (let i in this.config.students) {
        ...
        if (student.username) {
          ...
          console.log(
            "Error: Student '" + student.title + "' has an configuration error!"
          );
        }
      }
    }
  ...
  fetchLessonsAnonymous: function (studentData, days) {
    ...
    untis
      ...
      .then((response) => {
        ...
        return untis.getTimetableForRange(
          rangeStart,
          rangeEnd,
          classid,
  ...
  timetableToLessons: function (startTimes, timetable) {
    ...
    timetable.forEach((element) => {
      ...
      lesson.hour =
        lesson.hour.length == 3
          ? "0" + lesson.hour.substring(0, 1)
          : lesson.hour.substring(0, 2);
    ...
    if (this.config.debug) {
      console.log(
        "MMM-Webuntis: Timetable and Lessons: ",
        JSON.stringify({ timetable: timetable, lessons: lessons })
      );
    }
